...
import { savePaymentDetails, type CasePaymentDetails } from '@/app/actions/admin'
...
export function PaymentPanel({ caseId, status, servicePrice, savedDetails }: Props) {
  ...
  // Pre-fill from global settings if no saved details yet
  useEffect(() => {
  ...
  function handleSave() {
    ...
    startSave(async () => {
      try {
        await savePaymentDetails(caseId, details)
        setSaved(true)
        setEditing(false)
      } catch (e) {
    ...
    return (
      <div className="bg-white border border-navy/10 p-6">
        ...
        <p className="text-xs text-navy/60 mb-4 leading-relaxed">
          These details are visible to the client on their case page.
          The client can make the transfer and upload their payment screenshot.
        </p>
  ...
  return (
    <div className="bg-white border border-navy/10 p-6">
      <h2 className="font-serif text-lg text-navy mb-2">Send Payment Details to Client</h2>
      <p className="text-xs text-navy/60 mb-5 leading-relaxed">
        Fill in the bank details and deposit amount below. Once you click{' '}
        <strong>Send to Client</strong>, these appear on the client&apos;s case page so they
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="flex gap-3">
          ...
          <button
            ...
            onClick={handleSave}
            disabled={pending}
            className="flex-1 flex items-center justify-center gap-2 px-6 py-3 bg-navy text-white text-sm hover:bg-navy/90 active:scale-[0.98] transition-all disabled:opacity-60"
          >
            {pending
